package Koha::Plugin::Fi::KohaSuomi::DI::Koha::Item::Availability::Checkout;
...
sub in_intranet {
    ...
    unless ($patron = $self->patron) {
        Koha::Plugin::Fi::KohaSuomi::DI::Koha::Exceptions::MissingParameter->throw(
            ...
            .'requires Koha::Plugin::Fi::KohaSuomi::DI::Koha::Item::Availability::Checkout to have a patron '
            .'parameter.'
        );
    }
    ...
    if ($duedate &&
       ($reason = $checkoutcalc->invalid_due_date($item, $patron, $duedate))) {
        if (ref($reason) eq 'Koha::Plugin::Fi::KohaSuomi::DI::Koha::Exceptions::Checkout::DueDateBeforeNow') {
    ...
    my $checkoutrulecalc = Koha::Plugin::Fi::KohaSuomi::DI::Koha::Availability::Checks::CirculationRule->new({
        item => $item,
        patron => $patron,
    ...
    if ($reason = $itemcalc->checked_out($issue)) {
        if ($reason->borrowernumber == $patron->borrowernumber) {
            if ($reason = $checkoutcalc->no_more_renewals($issue)) {
                ...
                $self->confirm(Koha::Plugin::Fi::KohaSuomi::DI::Koha::Exceptions::Checkout::Renew->new);
            }
        } else {
    ...
    my $bibliocalc = Koha::Plugin::Fi::KohaSuomi::DI::Koha::Availability::Checks::Biblio->new($biblio);
    my $nomorerenewals = exists
       $self->unavailabilities->{'Koha::Plugin::Fi::KohaSuomi::DI::Koha::Exceptions::Checkout::NoMoreRenewals'}
       ? 1 : 0;
    my $renew = exists
       $self->confirmations->{'Koha::Plugin::Fi::KohaSuomi::DI::Koha::Exceptions::Checkout::Renew'}
       ? 1 : 0;
